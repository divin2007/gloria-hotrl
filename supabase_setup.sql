...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'room_status') THEN
        CREATE TYPE public.room_status AS ENUM ('Clean', 'Dirty', 'Maintenance', 'Occupied');
    END IF;
END $$;

-- 2. Core Operational Tables (Parent Tables First)
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='rooms' AND column_name='status') THEN
        ALTER TABLE public.rooms ADD COLUMN status public.room_status DEFAULT 'Clean';
    END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS public.inventory_items (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  name TEXT NOT NULL,
  category TEXT, -- 'Linens', 'Toiletries', 'Cleaning Supplies'
  current_stock INTEGER DEFAULT 0,
  unit TEXT, -- 'Units', 'Liters', 'Sets'
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- 3. Dependent Tables (Foreign Key Requirements Met)
...
CREATE TABLE IF NOT EXISTS public.tasks (
  ...
  assigned_to_id UUID REFERENCES auth.users ON DELETE SET NULL,
  room_number TEXT,
  task_time TEXT,
...
-- Inventory Usage (Depends on Tasks & Inventory Items)
...
-- Guest Charges (Depends on Reservations)
CREATE TABLE IF NOT EXISTS public.guest_charges (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  reservation_id UUID REFERENCES public.reservations(id) ON DELETE CASCADE,
  category TEXT NOT NULL, -- 'Room', 'Room Service', 'Spa', 'Laundry', 'Dining'
...
-- Pricing Log (Depends on Rooms)
CREATE TABLE IF NOT EXISTS public.pricing_log (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
-- Menu Items (Depends on Dining Venues)
...
-- Financial & Inventory Policies (Manager only)
DROP POLICY IF EXISTS "Managers can manage financials" ON public.guest_charges;
CREATE POLICY "Managers can manage financials" ON public.guest_charges FOR ALL USING (
...
CREATE POLICY "Staff can view inventory" ON public.inventory_items FOR SELECT USING (public.is_staff());
