...
const GuestDashboard = () => {
  ...
  const guestReservations = reservations.filter(res => res.guest === user?.email || res.guest === 'Website Lead' || res.guest === 'Guest from Web');
  const guestDining = diningReservations.filter(res => res.guest === user?.email);

  return (
    <div className="min-h-screen bg-background font-body text-on-surface p-8 lg:p-12">
      ...
      <header className="max-w-6xl mx-auto flex justify-between items-end mb-16">
        ...
        <button
          onClick={logout}
          className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant hover:text-secondary transition-colors border-b border-outline-variant pb-1"
        >
          Sign Out
        </button>
      </header>
